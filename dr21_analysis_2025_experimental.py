...
def bootstrap_fitting(array1, array2, error_array1, error_array2, Nbins, min_n=1, n_bootstrap=1000):
	...
	params_bootstrap = Parallel(n_jobs=-4, backend="loky")(
...
# 		# print('Bootstrap percent complete {p:.3f}%'.format(p = (i/n_bootstrap)*100))
...
# 		try:
# 			param, _, _ = fitting_binned(resampled_array1, resampled_array2, resampled_error1, resampled_error2, Nbins, min_n)
# 			params_bootstrap.append(param[1])
# 		except:
...
# 	bin_centres,binned_data,error_bar = binning_equal_width(resampled_array1,resampled_array2,resampled_error1,resampled_error2,Nbins,min_n = 1)
# 	bin_centres = np.log10(np.array(bin_centres))
# 	binned_data = np.log10(np.array(binned_data))
# 	valid = ~(np.isnan(bin_centres)|np.isnan(binned_data))
...
# def bootstrap_fitting_parallel(array1, array2, error_array1, error_array2, Nbins, min_n=1, n_bootstrap=1000):
# 	def lin_fit(x, a, b):
# 		return a + b*x
...
# 	try:
# 		param, _, _ = fitting_binned_bootstrap(resampled_array1, resampled_array2, resampled_error1, resampled_error2, Nbins)
# 		params_bootstrap.append(param[1])
# 	except:
# 		continue  # Skip failed fits

...
# set_delta = (2*19.2)/60   # in arcminute
...
# S_map_deb.writeto('../FITS_file/new_fits/S_map_deb_new_2025_38.fits')
# sigma_S_map.writeto('../FITS_file/new_fits/sigma_S_new_2025_38.fits')

#%%
S_map_deb =  fits.open('../FITS_file/new_fits/S_map_deb_new_2025_38.fits')[1]
sigma_S_map = fits.open('../FITS_file/new_fits/sigma_S_new_2025_38.fits')[1]
...
plt.tight_layout()
plt.savefig('../plots/2025/38/S_map_deb.png')
plt.show()
...
plt.ylabel('Number')
plt.savefig('../plots/2025/38/S_deb_hist.png')
plt.show()
...
plt.xlabel('debiased S')
plt.ylabel('Number')
plt.savefig('../plots/2025/38/sigma_S_hist.png')
plt.show()
# fig = plt.figure(figsize=(13,10))
...
def binning_equal_width_single(array1,array2,error_array1,error_array2,Nbins,min_n = 1):
	Nbins += 1 
...
def plotting_equal_width_bins(array1,array2,error_array1,error_array2,Nbins,min_n = 1,color_bins='#06402B',color_error='#8C564B'):
	def lin_fit(x, a, b):
		return a + b*x
	bin_centres,binned_data,error_bar = binning_equal_width_single(array1,array2,error_array1,error_array2,Nbins,min_n = 1)
	...
	# print(param)
	return param[0],level_bins


# def fitting_binned_single(array1,array2,error_array1,error_array2,Nbins,min_n = 1):
...
def get_wstd(vals, weights):
    ...
    variance = np.sum(weights * (vals - weighted_mean) ** 2) / (((len(weights) - 1) * np.sum(weights)) / len(weights))
    return np.sqrt(variance)

def binning_equal_width(array1, error_array1, array2_set, error_array2_set, Nbins, min_n=1):
    """
    Bins multiple datasets (array2_set) using the same array1 bin edges.
    
    Parameters:
        array1 (1D array): Values used for binning (x-axis).
        error_array1 (1D array): Errors associated with array1.
        array2_set (2D array): Shape (n, m), multiple datasets to be binned (y-axis).
        error_array2_set (2D array): Shape (n, m), corresponding errors.
        Nbins (int): Number of bins.
        min_n (int, optional): Minimum number of data points per bin.
        
    Returns:
        bin_centres (1D array): Centers of bins.
        binned_data (2D array): Binned values of array2_set.
        error_bar (2D array): Errors of the binned values.
    """
    Nbins += 1
    log_filtered1 = np.log10(array1)
    bins = np.linspace(np.nanmin(log_filtered1), np.nanmax(log_filtered1), Nbins)

    n_datasets = array2_set.shape[0]

    bin_centres = []
    binned_data = [[] for _ in range(n_datasets)]
    error_bar = [[] for _ in range(n_datasets)]

    for i in range(Nbins - 1):
        selector = (log_filtered1 > bins[i]) & (log_filtered1 < bins[i+1])

        if np.sum(selector) < min_n:
            bin_centres.append(np.nan)
            for j in range(n_datasets):
                binned_data[j].append(np.nan)
                error_bar[j].append(np.nan)
            continue

        bin_centre_temp = np.average(array1[selector], weights=1.0 / error_array1[selector] ** 2)
        bin_centres.append(bin_centre_temp)

        for j in range(n_datasets):
            temp_array2_fil = array2_set[j, selector]
            temp_error2_fil = error_array2_set[j, selector]

            weights = 1.0 / temp_error2_fil ** 2
            bin_data_temp = np.average(temp_array2_fil, weights=weights)
            binned_error = get_wstd(temp_array2_fil, weights=weights)

            binned_data[j].append(bin_data_temp)
            error_bar[j].append(binned_error)

    return np.array(bin_centres), np.array(binned_data), np.array(error_bar)

def fitting_binned(array1, error_array1, array2_set, error_array2_set, Nbins, min_n=1):
    """
    Fits multiple datasets using the same binning on array1.
    """
    def lin_fit(x, a, b):
        return a + b*x
    
    bin_centres, binned_data, error_bar = binning_equal_width(array1, error_array1, array2_set, error_array2_set, Nbins, min_n)
    bin_centres = np.log10(bin_centres)

    n_datasets = array2_set.shape[0]
    params_list = []
    
    for j in range(n_datasets):
        valid = ~(np.isnan(bin_centres) | np.isnan(binned_data[j]))
        param, _ = curve_fit(lin_fit, bin_centres[valid], np.log10(binned_data[j][valid]))
        params_list.append(param)

    return np.array(params_list)

def bootstrap_iteration(i, array1, error_array1, array2_set, error_array2_set, Nbins, min_n):
    """
    A single bootstrap iteration.
    """
    indices = np.random.choice(len(array1), len(array1), replace=True)
    resampled_array1 = array1[indices]
    resampled_error1 = error_array1[indices]
    resampled_array2_set = array2_set[:, indices]
    resampled_error2_set = error_array2_set[:, indices]

    try:
        params = fitting_binned(resampled_array1, resampled_error1, resampled_array2_set, resampled_error2_set, Nbins, min_n)
        return params[:, 1]  # Return slope values
    except:
        return None  # Skip failed fits

def bootstrap_fitting_parallel(array1, error_array1, array2_set, error_array2_set, Nbins, min_n=1, n_bootstrap=10000, n_jobs=-1):
	"""
	Parallelized bootstrap fitting for multiple datasets using Joblib.
	"""
	params_bootstrap = Parallel(n_jobs=n_jobs)(
		delayed(bootstrap_iteration)(i, array1, error_array1, array2_set, error_array2_set, Nbins, min_n)
		for i in range(n_bootstrap)
	)

	params_bootstrap = [param for param in params_bootstrap if param is not None]

	if len(params_bootstrap) == 0:
		raise ValueError("Bootstrap failed: No successful fits.")
	# print(len(params_bootstrap))
	params_bootstrap = np.array(params_bootstrap)
	params_bootstrap_trans = params_bootstrap.T
	# print(len(params_bootstrap_trans))

	for i in range(len(params_bootstrap_trans)):
		plt.figure()
		plt.hist(params_bootstrap_trans[i],50)
...
array3 = p_array
error_array3 = ep_array

array4 = s_array
error_array4 = es_array

array5 = p_array*s_array
error_array5 =  np.sqrt((p_array*es_array)**2+ (ep_array*s_array)**2)

array2_set = np.array([array2,array3,array4,array5])
error_array2_set = np.array([error_array2,error_array3,error_array4,error_array5])

# print(fitting_binned(array1,error_array1,array2_set,error_array2_set,10))

result = bootstrap_fitting_parallel(array1,error_array1,array2_set,error_array2_set,Nbins)


plt.figure(figsize=(12,8))
plt.scatter(array1,array2)
intercept,x = plotting_equal_width_bins(array1, array2,error_array1,error_array2,Nbins)
param_mean,param_std  = result[0][0],result[1][0]
y = lin_fit(x,intercept,param_mean)
x,y = 10**x,10**y
...
plt.tight_layout()
plt.legend()
plt.savefig('../plots/2025/38/I_vs_N.png')
plt.show()


plt.figure(figsize=(12,8))
plt.scatter(array1,array3)
intercept,x = plotting_equal_width_bins(array1, array3,error_array1,error_array3,Nbins)
param_mean,param_std  = result[0][1],result[1][1]
y = lin_fit(x,intercept,param_mean)
x,y = 10**x,10**y
...
plt.tight_layout()
plt.legend()
plt.savefig('../plots/2025/38/p_vs_N.png')
plt.show()

plt.figure(figsize=(12,8))
plt.scatter(array1,array4)
intercept,x = plotting_equal_width_bins(array1, array4,error_array1,error_array4,Nbins)
param_mean,param_std  = result[0][2],result[1][2]
y = lin_fit(x,intercept,param_mean)
x,y = 10**x,10**y
...
plt.tight_layout()
plt.legend()
plt.savefig('../plots/2025/38/S_vs_N.png')
plt.show()


plt.figure(figsize=(12,8))
plt.scatter(array1,array5)
intercept,x = plotting_equal_width_bins(array1, array5,error_array1,error_array5,Nbins)
param_mean,param_std  = result[0][3],result[1][3]
y = lin_fit(x,intercept,param_mean)
x,y = 10**x,10**y
...
plt.tight_layout()
plt.legend()
plt.savefig('../plots/2025/38/pS_vs_N.png')
plt.show()
...
array3 = s_array
error_array3 = es_array

array4 = p_array*s_array
error_array4 =  np.sqrt((p_array*es_array)**2+ (ep_array*s_array)**2)


array2_set = np.array([array2,array3,array4])
error_array2_set = np.array([error_array2,error_array3,error_array4])
...
plt.tight_layout()
plt.legend()
plt.savefig('../plots/2025/38/p_vs_I.png')
plt.show()
...
plt.tight_layout()
plt.legend()
plt.savefig('../plots/2025/38/S_vs_I.png')
plt.show()
...
plt.tight_layout()
plt.legend()
plt.savefig('../plots/2025/38/pS_vs_I.png')
plt.show()
...
array2_set = np.array([array2])
error_array2_set = np.array([error_array2])
...
plt.tight_layout()
plt.legend()
plt.savefig('../plots/2025/38/S_vs_p.png')
plt.show()
...
error_array3 = ep_C3_rest_array

array4 = s_C3_rest_array
error_array4 = es_C3_rest_array

array5 = p_C3_rest_array*s_C3_rest_array
error_array5 =  np.sqrt((p_C3_rest_array*es_C3_rest_array)**2+ (ep_C3_rest_array*s_C3_rest_array)**2)
...
plt.figure(figsize=(12,8))
plt.scatter(array1,array2,label = r'Not_OF')
intercept,x = plotting_equal_width_bins(array1, array2,error_array1,error_array2,Nbins)
param_mean,param_std  = result[0][0],result[1][0]
y = lin_fit(x,intercept,param_mean)
x,y = 10**x,10**y
...
plt.legend()
plt.tight_layout()
plt.savefig('../plots/2025/38/I_vs_N_notOF.png')
plt.show()


plt.figure(figsize=(12,8))
plt.scatter(array1,array3,label = r'Not_OF')
intercept,x = plotting_equal_width_bins(array1, array3,error_array1,error_array3,Nbins)
param_mean,param_std  = result[0][1],result[1][1]
y = lin_fit(x,intercept,param_mean)
x,y = 10**x,10**y
...
plt.legend()
plt.tight_layout()
plt.savefig('../plots/2025/38/p_vs_N_notOF.png')
plt.show()

plt.figure(figsize=(12,8))
plt.scatter(array1,array4,label = r'Not_OF')
intercept,x = plotting_equal_width_bins(array1, array4,error_array1,error_array4,Nbins)
param_mean,param_std  = result[0][2],result[1][2]
y = lin_fit(x,intercept,param_mean)
x,y = 10**x,10**y
...
plt.legend()
plt.tight_layout()
plt.savefig('../plots/2025/38/S_vs_N_notOF.png')
plt.show()



plt.figure(figsize=(12,8))
plt.scatter(array1,array5,label = r'Not_OF')
intercept,x = plotting_equal_width_bins(array1, array5,error_array1,error_array5,Nbins)
param_mean,param_std  = result[0][3],result[1][3]
y = lin_fit(x,intercept,param_mean)
x,y = 10**x,10**y
...
plt.xticks(np.array([1e22,1e23,1e24]))
plt.legend()
plt.savefig('../plots/2025/38/pS_vs_N_notOF.png')
plt.tight_layout()
plt.show()
...
error_array3 = es_C3_rest_array

array4 = p_C3_rest_array*s_C3_rest_array
error_array4 =  np.sqrt((p_C3_rest_array*es_C3_rest_array)**2+ (ep_C3_rest_array*s_C3_rest_array)**2)
...
plt.legend()
plt.tight_layout()
plt.savefig('../plots/2025/38/p_vs_I_notOF.png')
plt.show()
...
plt.legend()
plt.tight_layout()
plt.savefig('../plots/2025/38/S_vs_I_notOF.png')
plt.show()
...
plt.tight_layout()
plt.legend()
plt.savefig('../plots/2025/38/pS_vs_I_notOF.png')
plt.show()
#%%
...
plt.tight_layout()
plt.legend()
plt.savefig('../plots/2025/38/S_vs_p_notOF.png')
plt.show()
